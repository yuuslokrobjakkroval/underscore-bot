...
const event = {
  ...
  async execute(client, message) {
    ...
    if (mentionMatch) {
      ...
      // Check No-Prefix
      // If bot is private and user isn't an owner, ignore no-prefix triggers silently
      if (isPrivate(client) && !isOwner(client, message.author.id)) return;
      ...
      if (hasNoPrefixValue === undefined) {
        ...
        hasNoPrefixValue = await hasNoPrefix(
          client,
          message.author.id,
          userData,
        );
      ...
      if (hasNoPrefixValue) {
        ...
        if (exists) {
          client.logger.info(
            `[No-Prefix] User ${message.author.username} triggered command: ${commandName}`,
          );
        } else {
          commandName = "";
        }
    ...
    const botCheck = await resolveCommandBot(client, message, {
      command,
      isDirectMention,
    });
    if (!botCheck.allowed) {
    ...
    if (command.premium) {
      ...
      const isPremiumUserOrGuild =
        (await hasPremium(client, message.guild.id, message.author.id, {
          guildData,
          userData,
        })) || authorIsOwner;
