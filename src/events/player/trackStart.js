...
module.exports = {
  ...
  async execute(client, player, track) {
    client.logger.info(
      `Track started: ${track.title} in guild ${player.guildId}`,
    );
    ...
    if (voiceChannel && typeof voiceChannel.setStatus === "function") {
      voiceChannel
        .setStatus(`${emojis.feather} Playing: ${track.title}`)
        .catch(() => {});
    }
      ...
      User.findOneAndUpdate(
        { userId: requesterId },
        {
          $push: {
            history: {
              $each: [
                { title: track.title, author: track.author, uri: track.uri },
              ],
              $slice: -10,
            },
          },
        ...
        client.logger.error(
          `User History Error: ${err.message || JSON.stringify(err)}`,
        ),
      );
    }
    ...
    Guild.findOneAndUpdate(
      ...
      client.logger.error(
        `Guild History Error: ${err.message || JSON.stringify(err)}`,
      ),
    );
    ...
    try {
      ...
      client.logger.error(
        `Error handling player message: ${error.message || JSON.stringify(error)}`,
      );
    }

    // Update bot activity
    if (client.updateActivity) {
      client.updateActivity(client);
    }
  },
};
